...
sap.ui.core.mvc.Controller.extend("com.scs.view.Dashboard", {
	_oCatalog: null,
	...
	//	var that= this;//sap.ui.getCore().byId('idDashboard');
		...
		that._menu = new sap.ui.unified.Menu({
			items:[new sap.ui.unified.MenuItem({text:that._oResourceBundle.getText("Logout"),icon:"sap-icon://log",select:function(){app.back();}}),
			       ...
			       new sap.ui.unified.MenuItem({text:that._oResourceBundle.getText("AddUser"),icon:"sap-icon://add-contact",select:function(){app.to("idAdduser");}}),
...
//		https://www.twilio.com/user/billing
	...
	inboxReadSuccessCallbackInit: function(data) {
		...
		if ((data.indexOf("Fatal error") >= 0) || (data.indexOf("Warning")>=0)|| (data.indexOf("Notice")>=0)) {
			sap.m.MessageToast.show(this._oResourceBundle.getText("Inbox Items Load Failed"));
		}else{
			var oModel = new sap.ui.model.json.JSONModel();
